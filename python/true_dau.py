...
def myEventLoop(n):
    ...
    print dp_id
    E,P,M,S_y,S_x=[],[],[],[],[]
    #if abs(wg*xsw)<0.00000001: wg = wgnew(sTree.MCTrack[dp_id])
    for mc,track in enumerate(sTree.MCTrack):
        if track.GetMotherId()==1 and (sTree.MCTrack[dp_id].GetPdgCode()==9900015 or sTree.MCTrack[dp_id].GetPdgCode()==4900023):
            h['eff_P'].Fill(track.GetP(),wg*xsw)            
            h['eff_Pt'].Fill(track.GetPt(),wg*xsw)
            h['eff_Eta'].Fill(track.GetRapidity(),wg*xsw)
            X=track.GetStartX()
            Y=track.GetStartY()
            Z=track.GetStartZ()
            Px=track.GetPx()
            ...
            if abs(track.GetPdgCode())==11 or abs(track.GetPdgCode())==13 or abs(track.GetPdgCode())==15:
                if abs(track.GetPdgCode())==11:
                    e+=1
                    h['eff_P_e'].Fill(track.GetP(),wg*xsw)          
                    h['eff_Pt_e'].Fill(track.GetPt(),wg*xsw)
                    h['eff_Eta_e'].Fill(track.GetRapidity(),wg*xsw)
                    if isInFiducial(X,Y,Z):
                        e_v+=1
                ...
                if abs(track.GetPdgCode())==13:
                    mu+=1
                    h['eff_P_mu'].Fill(track.GetP(),wg*xsw)         
                    h['eff_Pt_mu'].Fill(track.GetPt(),wg*xsw)
                    h['eff_Eta_mu'].Fill(track.GetRapidity(),wg*xsw)
                    if isInFiducial(X,Y,Z):
                        mu_v+=1
                ...
                if abs(track.GetPdgCode())==15:#dont think that is the right way to do that
                    tau+=1
                    h['eff_P_tau'].Fill(track.GetP(),wg*xsw)            
                    h['eff_Pt_tau'].Fill(track.GetPt(),wg*xsw)
                    h['eff_Eta_tau'].Fill(track.GetRapidity(),wg*xsw)
                    if isInFiducial(X,Y,Z):     
                        tau_v+=1
            ...
            for daug in had:
                if track.GetMotherId()==daug:
                    ...
                    if abs(track.GetPdgCode())==211:
                        ...
                        S_x.append(a[0])
                        S_y.append(a[1])
                        h['eff_P_pi'].Fill(track.GetP(),wg*xsw)         
                        h['eff_Pt_pi'].Fill(track.GetPt(),wg*xsw)
                        h['eff_Eta_pi'].Fill(track.GetRapidity(),wg*xsw)
                        if isInFiducial(X,Y,Z):
                            pi_v+=1
                        ...
                        S_x.append(a[0])
                        S_y.append(a[1])
                        h['eff_P_ka'].Fill(track.GetP(),wg*xsw)         
                        h['eff_Pt_ka'].Fill(track.GetPt(),wg*xsw)
                        h['eff_Eta_ka'].Fill(track.GetRapidity(),wg*xsw)
                        if isInFiducial(X,Y,Z):
                            ka_v+=1
                        ...
                        if isInFiducial(X,Y,Z):
                             pi0_v+=1
                             h['eff_P0_pi0'].Fill(track.GetP(),wg*xsw)            
                             h['eff_Pt0_pi0'].Fill(track.GetPt(),wg*xsw)
                             h['eff_Eta0_pi0'].Fill(track.GetRapidity(),wg*xsw)
                             if ang_hcal(vtx,a):
                                 pi0_a+=1
    ...
    if dp==2:
        mass=invariant(S_x, S_y,E[0],E[1],P[0],P[1])
        h['eff_Pw'].Fill(sTree.MCTrack[dp_id].GetP(),wg*xsw)            
        h['eff_Ptw'].Fill(sTree.MCTrack[dp_id].GetPt(),wg*xsw)
        h['eff_Etaw'].Fill(sTree.MCTrack[dp_id].GetRapidity(),wg*xsw)
        h['DP'].Fill(mass)
        h['DPW'].Fill(mass,wg*xsw)
        if dp_v==2 and ((e==2 and e_v==2 and mu_v==0 and tau_v==0 and pi_v==0 and ka_v==0 and pi0_v==0  ) or (e_v==0 and mu==2 and  mu_v==2 and tau_v==0 and pi_v==0 and ka_v==0 and pi0_v==0  ) or (e_v==0 and mu_v==0 and tau==2 and tau_v==2 and pi_v==0 and ka_v==0 and pi0_v==0  ) or (pi==2 and pi0==0 and e_v==0 and mu_v==0 and tau_v==0 and pi_v==2 and ka_v==0 and pi0_v==0  ) or (e_v==0 and mu_v==0 and tau_v==0 and pi_v==0 and ka==2 and ka_v==2 and pi0_v==0  ) or (e_v==0 and mu_v==0 and tau_v==0 and pi==2 and pi0==1 and  pi_v==2 and ka_v==0 and pi0_v==1  ) or (pi==2 and pi0==2 and e_v==0 and mu_v==0 and tau_v==0 and pi_v==2 and ka_v==0 and pi0_v==2  ) or (pi==4 and e_v==0 and mu_v==0 and tau_v==0 and pi_v==4 and ka_v==0 and pi0_v==0)):
            h['DPves'].Fill(mass)
            h['DPvesW'].Fill(mass,wg*xsw)
            h['eff_P0w'].Fill(sTree.MCTrack[dp_id].GetP(),wg*xsw)           
            h['eff_Pt0w'].Fill(sTree.MCTrack[dp_id].GetPt(),wg*xsw)
            h['eff_Eta0w'].Fill(sTree.MCTrack[dp_id].GetRapidity(),wg*xsw)
            if dp_a==2 and ((e_a==2 and e_v==2 and mu_a==0 and tau_a==0 and pi_a==0 and ka_a==0 and pi0_a==0  ) or (e_a==0 and mu_v==2 and  mu_a==2 and tau_a==0 and pi_a==0 and ka_a==0 and pi0_a==0  ) or (e_a==0 and mu_a==0 and tau_a==2 and tau_v==2 and pi_a==0 and ka_a==0 and pi0_a==0  ) or (pi0_v==0 and pi_v==2 and e_a==0 and mu_a==0 and tau_a==0 and pi_a==2 and ka_a==0 and pi0_a==0  ) or (e_a==0 and mu_a==0 and tau_a==0 and pi_a==0 and ka_a==2 and ka_v==2  and pi0_a==0  ) or (e_a==0 and mu_a==0 and tau_a==0 and pi_a==2 and ka_a==0 and pi0_a==1 and pi_v==2 and pi0_v==1 ) or (pi_v==2 and  pi0_v==2 and e_a==0 and mu_a==0 and tau_a==0 and pi_a==2 and ka_a==0 and pi0_a==2  ) or (e_a==0 and mu_a==0 and tau_a==0 and pi_a==4 and ka_a==0 and pi0_a==0)):
                h['DPang'].Fill(mass)
                h['DPangW'].Fill(mass,wg*xsw)
                h['eff_P1w'].Fill(sTree.MCTrack[dp_id].GetP(),wg*xsw)           
                h['eff_Pt1w'].Fill(sTree.MCTrack[dp_id].GetPt(),wg*xsw)
                h['eff_Eta1w'].Fill(sTree.MCTrack[dp_id].GetRapidity(),wg*xsw)
...
#if float(h['DPves'].Integral())!=0: Acc=float(h['DPang'].Integral())/float(h['DPves'].Integral())
