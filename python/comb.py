...
data1  = open(pathW+'qcd_Ana_rate1.dat', header=None)
data2  = open(pathW+'meson_Ana_rate1.dat', header=None)
data3  = open(pathW+'pbrem_Ana_rate1.dat', header=None)

sum1   = open(pathR+'meson_Ana_sum.dat','r')
sum2   = open(pathR+'pbrem_Ana_sum.dat','r')
sum3   = open(pathR+'qcd_Ana_sum.dat','r')
Rate   = open(pathR+'Rate1.dat','w')
...
def find_N(lines,eps,mass):
    for i in lines:
        k = i.replace('\n','')
        k = k.split(' ')
        if abs(math.log10(float(k[1])) + eps) <0.1 and abs(float(k[0]) - mass)<0.0001: return float(k[4])
    else:
