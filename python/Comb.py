...
if not wo:
    ...
    if lepto:
        ...
        data2  = open(pathW+'pbrem1_Ana_rate2.dat')
        Rate   = open(pathW+'comb1_Ana_rate2.dat','w')
...
def find_Rate(lines,mass,eps):
    for i in lines:
        ...
        #print k
        k = k.split(' ')
        if abs(math.log10(Decimal(k[1])) - math.log10(eps)) <0.1 and abs(Decimal(k[0]) - mass)<0.0001: return Decimal(k[2])
    else: return 0
