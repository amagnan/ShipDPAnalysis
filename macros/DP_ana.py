...
try:
	opts, args = getopt.getopt(sys.argv[1:], "d:p:m:e:A:g:f:C:zmin:zmax:ptmax:", ["date=","production=","mass=","epsilon=","motherID=","geoFile=","final_dest=","cascade=","zmin=","zmax=","ptmax="])
...
if dpMom=='':
	...
	if "pbrem" in pro:
		if	zmin!='' and zmax!='' and ptmax!='': tmp1 = "/eos/experiment/ship/data/DarkPhoton/PBC-June-3/"+date+"/reco/pbrem_mass"+mass_mc+"_eps"+eps+"_zmin"+zmin+"_zmax"+zmax+"_ptmax"+ptmax
	...
	for dp_ind,dp_tr in enumerate(sTree.MCTrack):
		if dp_tr.GetPdgCode()==9900015 or dp_tr.GetPdgCode()==4900023:
			...
			if pro=='meson':
				...
				if pro=='qcd': xsw = dputil.getDPprodRate(mass_mc,eps,pro,0)
				if 'pbrem' in pro: 
					if	zmin!='' and zmax!='' and ptmax!='': xsw = dputil.getDPprodRate(mass_mc,eps,pro,0)
