...
int plotSensitivityQl(){
  ...
  const unsigned nP = 1;
  std::string proc[nP] ={"qcd_SigX_true"};
  ...
  for (unsigned iP(0); iP<nP; ++iP){
    ...
    hvtxEff[iP] = new TH2F(lname.str().c_str(),";mass (GeV);#gamma' coupling to SM log_{10}(#varepsilon);Geometrical Acceptance (%)",120,0,9.2,100,-9,-5);
    ...
    if (gr) {
      ...
      for (unsigned ieps(0); ieps<1000000;++ieps){
        double tmpeps= - 5.5 +ieps*3./1000000;
      ...
      if (limitVec.size()>0){
        ...
        unsigned inext = 1;
        while (fabs(limitVec[inext].second-limitVec[0].second)<0.0001) {
          inext++;
