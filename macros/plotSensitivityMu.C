...
static bool customSort(const Rate & a,
               const Rate & b)
{
  if (fabs(a.mass-b.mass)<0.001) return a.eps <= b.eps;
...
void readInput(const std::string & prodfile,
  ...
  while(1){
    ...
    if (tmp.mass>0.00000 && tmp.rate2>0){
...
int plotSensitivityMu(){
  ...
  const std::string prod="/afs/cern.ch/user/t/takmete/ShipDPAnalysis/data/w180819/"; 
  TLatex lat;
  char lbuf[500];
  ...
  const unsigned nP = 1;
  std::string proc[nP] ={"meson_SigX_true"};
  ...
  for (unsigned iP(0); iP<nP; ++iP){
    ...
    hSens[iP] = new TH2F(lname.str().c_str(),";mass (GeV);#gamma' coupling to SM log_{10}(#varepsilon);rate for 2.10^{10} p.o.t.",120,0,1.0,100,-9,-4);
    if (!hSens[iP]) return 1;

    lname.str("");
    lname << "hvtxEff_" << proc[iP];
    hvtxEff[iP] = new TH2F(lname.str().c_str(),";mass (GeV);#gamma' coupling to SM log_{10}(#varepsilon);Geometrical Acceptance (%)",120,0,1.0,100,-9,-4);
    ...
    for (unsigned iN(0); iN<lRate[iP].size(); ++iN){
      ...
      //IMPORTANT
      if (fabs(lRate[iP][iN].mass-mass)>0.001 || iN==lRate[iP].size()-1) {
    ...
    if (gr) {
      ...
      //line -10,-3 
      line = new TLine(-9,log10(clsval),-3,log10(clsval));
      line->SetLineColor(6);
      line->Draw("same");
      ...
      for (unsigned ieps(0); ieps<1000000;++ieps){
        double tmpeps=-6.0+ieps*8./1000000;
      ...
      if (limitVec.size()>0){
        ...
        unsigned inext = 1;
        while (fabs(limitVec[inext].second-limitVec[0].second)<0.001) {
          inext++;
        ...
        line1 = new TLine(limitVec[0].second,-6,limitVec[0].second,6);
        line1->SetLineColor(6);
        line1->Draw("same");
        line2 = new TLine(limitVec[inext].second,-6,limitVec[inext].second,6);
