...
int plotSensitivityPu(){
  ...
  const unsigned nP = 1;
  std::string proc[nP] ={"pbrem_SigX_true"};
  ...
  for (unsigned iP(0); iP<nP; ++iP){
    ...
    hSens[iP] = new TH2F(lname.str().c_str(),";mass (GeV);#gamma' coupling to SM log_{10}(#varepsilon);rate for 2.10^{10} p.o.t.",120,0,2.3,100,-9,-4);
    if (!hSens[iP]) return 1;

    lname.str("");
    lname << "hvtxEff_" << proc[iP];
    hvtxEff[iP] = new TH2F(lname.str().c_str(),";mass (GeV);#gamma' coupling to SM log_{10}(#varepsilon);Geometrical Acceptance (%)",120,0,2.3,100,-9,-4);
    ...
    if (gr) {
      ...
      for (unsigned ieps(0); ieps<100000;++ieps){
        double tmpeps=-6.0+ieps*5./100000;
